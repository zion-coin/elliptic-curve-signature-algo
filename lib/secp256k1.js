...
function generateSignature(message, privKey) {
  // Ensure proper format
  if (typeof message === 'string') message = bigInt(message, 16);
  if (typeof privKey === 'string') privKey = bigInt(privKey, 16);
...
function verifySignature(message, RandSignPoint, signature) {
  if (typeof message === 'string') message = bigInt(message, 16);
  if (typeof signature === 'string') signature = bigInt(signature, 16);
...
function PublicKeyGenerate(PrivateKey) {
  ...
  let compressed = modulo(PublicKey[1], 2).eq(1) ? "03" + Px : "02" + Px;
...
function doubleSha256(secret) {
  ...
  return crypto.createHash('sha256').update(Buffer.from(hash, 'hex')).digest('hex');
}

function ripemd160(secret) {
  if (typeof secret === 'string') secret = Buffer.from(secret, 'hex');

  return new RIPEMD160().update(secret).digest('hex');
}
